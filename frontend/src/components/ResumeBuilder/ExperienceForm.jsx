  ...
  const handleGenerateDescription = async (index) => {
    ...
    if (
      ...
      handleChange(
        index,
        "description",
        "Please enter company name and position first to generate a description."
      );
      return;
    }
    ...
    const prompt = `Write 3-4 professional resume bullet points for a ${position} at ${company}${
      location ? " in " + location : ""
    }${dateRange}. Each bullet point should start with a strong action verb and highlight key responsibilities, achievements, and impact. Make them specific, quantifiable where possible, and suitable for a resume. Format as bullet points with • prefix.`;
    ...
    try {
      ...
      if (!HF_TOKEN || HF_TOKEN === "YOUR_HUGGINGFACE_TOKEN_HERE") {
        handleChange(
          index,
          "description",
          "Error: HuggingFace API token not configured. Please add REACT_APP_HF_TOKEN to your environment variables and redeploy."
        );
        console.error(
          "REACT_APP_HF_TOKEN is not set. Value:",
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          handleChange(
            index,
            "description",
            "Authentication error: Invalid HuggingFace API token. Please check your token."
          );
